...
This means that there 6 data gaps with \$75 - 15 / 15 = \`r 75 - 15 / 15\`\$ missing rows per data gap.
...
```{r}
data_power_prod_ts_long |>
  mutate(weekday = lubridate::wday(date, label = TRUE, abbr = TRUE)) |> mutate(weekday = as_factor())
